...
export default class Objects extends React.Component {
    ...
    render() {
        const { house, i, mapStyles, updateInputTextHouse, updateCoordinatesByString, onUpdateInputTextPoint, onAddPoint, onRemovePoint } = this.props;
            ...
            <div className='form-group col-sm-12 col-md-12'>
                ...
                <div className={'row ' + styles.line}>
                    ...
                    <div className={'col-xs-4 ' + styles.apply_path_by_str}>
                        ...
                        <button type='button' className='btn btn-info' onClick={() => { updateCoordinatesByString(this.refs.coorStr); }}><span className='glyphicon glyphicon-plus-sign'></span></button>
                    </div>
                    <div className='col-xs-4'>
                        <div className='row'>
                ...
                {house.coordinates.map((latLong, j) => {
                    return <div className='row form-group' key={'house_' + i + '_' + j}>
                        <div className='col-xs-6'>
                            <input type='text' className='form-control' id='lat' placeholder='Широта' value={latLong.lat} onChange={e => onUpdateInputTextPoint(e, j)} />
                        </div>
                        <div className='col-xs-6'>
                            <input type='text' className='form-control' id='lng' placeholder='Долгота' value={latLong.lng} onChange={e => onUpdateInputTextPoint(e, j)} />
                        </div>
                    </div>
                })}
                <div className={'row ' + styles.line}>
                    <div className='col-xs-12'>
                        <button type='button' className='btn btn-info' onClick={() => { onAddPoint(); }}><span className='glyphicon glyphicon-plus-sign'></span></button>
                        &nbsp;
                        <button type='button' className='btn btn-danger' onClick={() => { onRemovePoint(); }}><span className='glyphicon glyphicon-minus-sign'></span></button>
                    </div>
                </div>
            </div>
        </div>
